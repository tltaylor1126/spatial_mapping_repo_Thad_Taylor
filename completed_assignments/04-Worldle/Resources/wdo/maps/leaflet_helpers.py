...
def _is_blank_basemap(name):
    return isinstance(name, str) and name.lower() in {"blank", "none", "silhouette"}


def _basemap_from_name(leaflet, name):
    if name is None:
        return leaflet.basemaps.CartoDB.Positron
    if _is_blank_basemap(name):
        return _blank_tile_layer(leaflet)
...
def make_map(center=(0, 0), zoom=2, basemap=None, scroll_wheel_zoom=True, **kwargs):
    ...
    leaflet = _leaflet()
    if _is_blank_basemap(basemap):
        layers = kwargs.pop("layers", None) or (_blank_tile_layer(leaflet),)
        return leaflet.Map(
            center=center,
            zoom=zoom,
            layers=layers,
