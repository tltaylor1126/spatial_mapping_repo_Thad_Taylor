...
def _shift_coordinates(value, source_mid_lon, source_mid_lat, target_lon, target_lat):
    if isinstance(value, (list, tuple)) and len(value) >= 2:
        first, second = value[0], value[1]
        if isinstance(first, (int, float)) and isinstance(second, (int, float)):
            shifted = [
                first - source_mid_lon + target_lon,
                second - source_mid_lat + target_lat,
            ]
            if len(value) > 2:
    ...
    if isinstance(value, (list, tuple)):
        return [
            _shift_coordinates(item, source_mid_lon, source_mid_lat, target_lon, target_lat)
            for item in value
        ]
    return value


def silhouette_feature(feature, center=(0, 0)):
    """Return a display-only copy of a country feature as a centered silhouette.

    The returned GeoJSON keeps only the target geometry, removes all identifying
    properties, and translates the coordinates so the shape is not drawn at its
    real world location.
    """
    geometry = feature.get("geometry")
    if not geometry:
        raise ValueError("silhouette_feature requires a GeoJSON feature with geometry")

    min_lon, min_lat, max_lon, max_lat = bbox_from_feature(feature)
    source_mid_lon = (min_lon + max_lon) / 2
    source_mid_lat = (min_lat + max_lat) / 2
    ...
    return {
        ...
        "geometry": {
            ...
            "coordinates": _shift_coordinates(
